...
export function AdminInstructors() {
  ...
  const openCreate = () => { setForm({ firstName: '', lastName: '', bio: '' }); setIsCreating(true) }
  const openEdit = (i: Instructor) => { setForm({ firstName: i.firstName, lastName: i.lastName, bio: i.bio ?? '' }); setEditItem(i) }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold text-surface-100">{t('instructors.title')}</h2>
        <Button variant="primary" size="sm" onClick={openCreate}>{t('actions.create')}</Button>
      </div>

      {!instructors?.length ? (
        <p className="text-surface-500">{t('instructors.noItems')}</p>
      ) : (
        <div className="space-y-3">
          {instructors.map((instr, idx) => (
            <div key={instr.id} className={clsx('flex items-center gap-4 bg-surface-900 border border-surface-800 rounded-xl p-4', !instr.active && 'opacity-50')}>
              <div className="w-12 h-12 rounded-full bg-surface-800 flex items-center justify-center overflow-hidden flex-shrink-0">
                {instr.photoUrl ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                {instr.bio && <p className="text-sm text-surface-500 truncate">{instr.bio}</p>}
              </div>
              <div className="flex items-center gap-1">
                <label className="cursor-pointer">
                  ...
                  />
                  <span className="px-2 py-1 text-xs bg-surface-800 text-surface-300 rounded hover:bg-surface-700 transition-colors">{t('actions.upload')}</span>
                </label>
      ...
      <Modal
        ...
        title={editItem ? t('instructors.editTitle') : t('instructors.createTitle')}
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-surface-300 mb-1">{t('instructors.firstName')}</label>
            ...
            <div>
              <label className="block text-sm font-medium text-surface-300 mb-1">{t('instructors.lastName')}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-surface-300 mb-1">{t('instructors.bio')}</label>
