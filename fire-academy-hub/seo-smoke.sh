...
#   - the nginx bot-UA meta-refresh LOOP (a search crawler wrongly added to the
#     bot list gets bounced /treningi/termin/<id> -> /og/... -> meta-refresh ->
#     /treningi/termin/<id> forever -> GSC "Redirect error" -> pages deindexed),
...
#
# Usage:  ./seo-smoke.sh [BASE_URL]      (default: https://fireworkout.pl)
# Exit:   0 = all checks passed, 1 = at least one check failed.

set -uo pipefail

BASE="${1:-https://fireworkout.pl}"
...
# correct as content changes (no hardcoded IDs). Detail URLs are either
# /<treningi|obozy|szkolenia>/<rodzaj|termin>/<uuid> or /kadra/<uuid>.
DETAIL_URL=$(echo "$SITEMAP" | grep -oE "https?://[^<]+/((treningi|obozy|szkolenia)/(rodzaj|termin)|kadra)/[a-f0-9-]+" | head -1)
...
if [ -z "$DETAIL_URL" ]; then
  ...
  # A meta-refresh served to Googlebot is the exact signature of the bot-UA
  # loop that deindexes pages.
  G_BODY=$(get -A "$GOOGLE_UA" "${BASE}${DPATH}" || true)
  if echo "$G_BODY" | grep -qi 'http-equiv="refresh"'; then
    fail "search crawler got a meta-refresh — bot-UA LOOP (GSC redirect error risk)"
  else
