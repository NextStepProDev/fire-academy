...
- Rate limiting: **per-IP per-kubełek**, nie per-endpoint. `RateLimitFilter` ma 6 kubełków rozstrzyganych prefiksem ścieżki: `auth` 15/min · **`upload` 12/min** · `user` 20/min · **`mytraining` 120/min** · `admin` 60/min · `public` 120/min. Kalendarz 1:1 ma własny kubełek, bo samo przeglądanie generuje więcej żądań niż cała reszta `/api/user/**` razem wzięta. Kubełek `upload` (`/api/user/my-training/photos`, `/api/admin/training-photos`) racjonuje **bajty, nie żądania** — multipart jest parsowany do pamięci, zanim handler zdąży cokolwiek odrzucić; **odczyt zdjęć celowo w nim nie siedzi**. ⚠️ Prefiksy w `resolveLimit` i `resolveBucket` muszą być sprawdzane w **tej samej kolejności, od najbardziej szczegółowego** — inaczej limit jednego kubełka trafi na licznik drugiego
